...
On the `storage` VM in a second terminal, add files:

```bash
vagrant ssh storage

echo a | sudo tee /srv/exorg_exsrv/data/exsrv/ms-data/ag-alice/a.dat
echo b | sudo tee /srv/exorg_exsrv/data/exsrv/ms-data/ag-alice/b.dat
```

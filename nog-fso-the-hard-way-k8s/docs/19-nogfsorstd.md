...
```bash
mkdir -p local/k8s/secrets/nogfsorstd-etc
cp local/pki/tls/ca.pem local/k8s/secrets/nogfsorstd-etc/fso-tls-cabundle.pem
cp local/pki/tls/nogfsorstd-combined.pem local/k8s/secrets/nogfsorstd-etc/fso-tls-combined.pem
```
...
| jq -r .data.token | tee local/k8s/secrets/nogfsorstd-etc/nogfsorstd.jwt
{
    "expiresIn": 5443200,
...
```bash
(
    cd local/k8s/secrets/nogfsorstd-etc && \
    kubectl create secret generic \
        nogfsorstd-etc $(printf -- '--from-file=%s ' *.pem *.jwt) \
        --dry-run -o yaml
) \
| kubectl apply -f -
```

Update the `storage` stateful set to start `nogfsorstd`:
...
```bash
kubectl get pods -w
kubectl logs storage-0 -c nogfsorstd
```

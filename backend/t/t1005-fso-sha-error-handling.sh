...
# `--jwt-auth`, because the Meteor app does not know the repo.
test_expect_success 'sha error is stored as repo error' '
    touch "d a t/foo/2" &&
    chmod a-rwx "d a t/foo/2" &&
    : &&
    nogfsoctl sha --jwt-auth=no --author="A U Thor <author@example.com>" "${repoId}" &&
    : &&
    waitGrep 5s "error.*git-fso sha failed" stad.log &&

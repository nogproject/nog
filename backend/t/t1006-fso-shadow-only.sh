...
test_description='
Basic fso shadow-only; see also t1001
'
...
test_expect_success 'registry events' '
    [ $(nogfsoctl events registry exreg | wc -l) = 4 ] &&
...
test_expect_success 'repo events' '
    [ $(nogfsoctl events repo --jwt-auth=no "${repoId}" | wc -l) = 2 ] &&
...
test_expect_success 'stat' '
    ...
    waitGrep 10s "Skipped push.*${repoId}" stad.log &&
    true
'

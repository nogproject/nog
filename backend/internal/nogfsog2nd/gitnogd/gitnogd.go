...
func (srv *Server) Head(ctx context.Context, i *pb.HeadI) (*pb.HeadO, error) {
	...
	glc, projectId, err := srv.findGitlab(repoId)
	if err != nil {
		return nil, err
	}
...
func (srv *Server) findGitlab(repo uuid.I) (*gitlab.Client, int, error) {
	gi := srv.registry.GitlabProjectInfo(repo)
	if gi == nil {
	...
	glc, ok := srv.gitlabs[gi.Hostname]
	if !ok {
		err := status.Errorf(codes.Internal, "missing GitLab client")
		srv.lg.Errorw(
			"logic error: missing gitlab for repo",
			"repo", repo,
		)
		return nil, 0, err
	}

	if gi.EnabledSince.After(time.Now().Add(-enableGitlabDeadtime)) {
		err := status.Errorf(
			codes.FailedPrecondition,

...
func New(
	...
	return &Server{
		...
		lg:          lg,
		prefixes:    pfxs,
		reposClient: pb.NewReposClient(regConn),
		gitlabs:     gls,
...
func (srv *Server) Head(ctx context.Context, i *pb.HeadI) (*pb.HeadO, error) {
	repoId, err := uuid.FromBytes(i.Repo)
	if err != nil {
		err = status.Errorf(
	...
	glc, projectId, err := srv.findGitlab(ctx, repoId)
	if err != nil {
		return nil, err
	}
...
func (srv *Server) findGitlab(
...
) (*gitlab.Client, int, error) {
	repo, err := srv.reposClient.GetRepo(ctx, &pb.GetRepoI{
		Repo: repoId[:],
	})
	if err != nil {
		return nil, 0, err
	...
	projectId := int(repo.GitlabProjectId)

	host := strings.SplitN(repo.Gitlab, ":", 2)[0]
	glc, ok := srv.gitlabs[host]
	if !ok {

...
func cmdRepoBeginArchive(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	ctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()

	c := pb.NewArchiveRepoClient(conn)
	...
	creds, err := getRPCCredsRepoId(ctx, args, AAFsoArchiveRepo, repoId)
	if err != nil {
		lg.Fatalw("Failed to get auth token.", "err", err)
	}
	o, err := c.BeginArchiveRepo(ctx, i, creds)
	if err != nil {
...
func cmdRepoGetArchive(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	...
	o, err := c.GetArchiveRepo(ctx, i, creds)
	if err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}
...
func cmdRepoArchive(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	timeout, optWait := args["--wait"].(time.Duration)
	if !optWait {
		timeout = 10 * time.Second
	...
	if _, err := c.BeginArchiveRepo(ctx, beginI, creds); err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}

	getI := &pb.GetArchiveRepoI{
		Workflow:   workflowId[:],
		JobControl: pb.JobControl_JC_WAIT,
	}
	o, err := c.GetArchiveRepo(ctx, getI, creds)
	if err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}

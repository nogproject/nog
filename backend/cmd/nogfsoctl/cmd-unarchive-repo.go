...
func cmdRepoBeginUnarchive(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	ctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()

	c := pb.NewUnarchiveRepoClient(conn)
	...
	creds, err := getRPCCredsRepoId(ctx, args, AAFsoUnarchiveRepo, repoId)
	if err != nil {
		lg.Fatalw("Failed to get auth token.", "err", err)
	}
	o, err := c.BeginUnarchiveRepo(ctx, i, creds)
	if err != nil {
...
func cmdRepoGetUnarchive(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	...
	o, err := c.GetUnarchiveRepo(ctx, i, creds)
	if err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}
...
func cmdRepoUnarchive(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	timeout, optWait := args["--wait"].(time.Duration)
	if !optWait {
		timeout = 10 * time.Second
	...
	if _, err := c.BeginUnarchiveRepo(ctx, beginI, creds); err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}

	getI := &pb.GetUnarchiveRepoI{
		Workflow:   workflowId[:],
		JobControl: pb.JobControl_JC_WAIT,
	}
	o, err := c.GetUnarchiveRepo(ctx, getI, creds)
	if err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}

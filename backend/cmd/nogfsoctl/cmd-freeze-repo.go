...
func cmdRepoBeginFreeze(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	ctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()

	c := pb.NewFreezeRepoClient(conn)
	...
	creds, err := getRPCCredsRepoId(ctx, args, AAFsoFreezeRepo, repoId)
	if err != nil {
		lg.Fatalw("Failed to get auth token.", "err", err)
	}
	o, err := c.BeginFreezeRepo(ctx, i, creds)
	if err != nil {
...
func cmdRepoGetFreeze(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	...
	o, err := c.GetFreezeRepo(ctx, i, creds)
	if err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}
...
func cmdRepoFreeze(args map[string]interface{}, conn *grpc.ClientConn) {
	ctx := context.Background()
	timeout, optWait := args["--wait"].(time.Duration)
	if !optWait {
		timeout = 10 * time.Second
	...
	if _, err := c.BeginFreezeRepo(ctx, beginI, creds); err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}

	getI := &pb.GetFreezeRepoI{
		Workflow:   workflowId[:],
		JobControl: pb.JobControl_JC_WAIT,
	}
	o, err := c.GetFreezeRepo(ctx, getI, creds)
	if err != nil {
		lg.Fatalw("RPC failed.", "err", err)
	}

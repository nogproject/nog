...
function createFsoTokenProvider(settings) {
  ...
  function sign(payload, { userId, expiresIn, name }) {
  ...
  function fsoToken(user, opts) {
    ...
    const xcrd = [];
    Object.entries(user.services).forEach(([service, v]) => {
  ...
  function fsoSysTokenDetailed(user, opts) {
    check(user, Match.ObjectIncluding({
      _id: Match.Optional(String),
      username: String,
    }));
  ...
  function fsoSysToken(...args) {
    return fsoSysTokenDetailed(...args).token;
  }

  function deleteUserToken(userId, opts) {
    check(userId, String);
    check(opts, {
      jti: String,
    });
    ...
    users.update(userId, {
      $pull: { 'services.nogfsoiam.jwts': { jti } },
    });
  }

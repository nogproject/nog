...
function getSettingsWellknownAccounts() {
  ...
  console.log(
    '[oidc-testapp] Using wellknownAccounts dev settings:', devSettings,
  );
  return devSettings;
}
...
function getSettingsLdap() {
  ...
  console.log('[oidc-testapp] Using ldap dev settings:', devSettings);
  console.log(
    '[oidc-testapp] Use an SSH tunnel for testing gitimp: '
    + 'ssh -L localhost:13389:ldap.imp.fu-berlin.de:389 login.imp',
  );
  console.log(
    '[oidc-testapp] Use an SSH tunnel for testing gitzib: '
...
Accounts.onCreateUser((opts, partialUser) => {
  ...
  // client to login again.  The next login will resolve to the existing user.
